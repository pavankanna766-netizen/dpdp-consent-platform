...
export async function GET(request: NextRequest) {
  try {
    ...
    return successResponse(await bannerConsentService.receipt(query.bannerToken, query.visitorId), 200, {
      headers: publicBannerHeaders({ "Cache-Control": "no-store" }),
    });
  } catch (error) {
    if (error instanceof ZodError) {
      return withPublicBannerHeaders(validationErrorResponse("Invalid consent receipt request.", error.issues));
    }
